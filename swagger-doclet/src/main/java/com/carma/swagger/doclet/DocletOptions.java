...
public class DocletOptions {
  ...
  private static <T> T loadModelFromJson(String option, String path,
      Class<T> resourceClass) {
    File file = new File(path);
    checkArgument(file.isFile(),
        "Path after " + option + " (" + file.getAbsolutePath()
    ...
    try {
      ...
      throw new IllegalArgumentException("Failed to read model file: " + path
  ...
  public static DocletOptions parse(String[][] options) {
    ...
    for (String[] option : options) {
      if (option[0].equals("-d")) {
        ...
        checkArgument(parsedOptions.outputDirectory.isDirectory(),
            "Path after -d is expected to be a directory!");
      } else if (option[0].equals("-apiAuthorizationsFile")) {
        parsedOptions.apiAuthorizations =
            loadModelFromJson("-apiAuthorizationsFile", option[1],
                ApiAuthorizations.class);
      } else if (option[0].equals("-apiInfoFile")) {
        parsedOptions.apiInfo =
            loadModelFromJson("-apiInfoFile", option[1], ApiInfo.class);
      } else if (option[0].equals("-variablesPropertiesFile")) {

        File varFile = new File(option[1]);
        if (!varFile.exists() && !varFile.canRead()) {
          throw new IllegalStateException("Unable to read variables file: "
              + varFile.getAbsolutePath() + " check it exists and is readable.");
        }
        ...
        try {
          ...
          throw new IllegalStateException("Failed to read variables file: "
              + varFile.getAbsolutePath(), ex);
        } finally {
          if (is != null) {
            try {
              is.close();
        ...
        parsedOptions.apiVersion = option[1];
          ...
          || option[0].equals("-swaggerUiPath")) {
        parsedOptions.swaggerUiPath = option[1];
      } else if (option[0].equals("-responseMessageSortMode")) {
        parsedOptions.responseMessageSortMode =
            ResponseMessageSortMode.valueOf(option[1]);
      } else if (option[0].equals("-excludeParamAnnotations")) {
        parsedOptions.excludeParamAnnotations.addAll(asList(copyOfRange(option,
            1, option.length)));
        ...
        parsedOptions.modelFieldsNamingConvention =
            NamingConvention.forValue(option[1], NamingConvention.DEFAULT_NAME);
      } else if (option[0].equals("-disableCopySwaggerUi")
          || option[0].equals("-skipUiFiles")) {
        ...
        parsedOptions.responseMessageTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-excludeModelPrefixes")
          || option[0].equals("-typesToTreatAsOpaque")) {
        parsedOptions.excludeModelPrefixes.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-excludeResourcePrefixes")) {
        parsedOptions.excludeResourcePrefixes.addAll(asList(copyOfRange(option,
            1, option.length)));
      } else if (option[0].equals("-genericWrapperTypes")) {
        parsedOptions.genericWrapperTypes.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-fileParameterAnnotations")) {
        parsedOptions.fileParameterAnnotations.addAll(asList(copyOfRange(
            option, 1, option.length)));
      } else if (option[0].equals("-fileParameterTypes")) {
        parsedOptions.fileParameterTypes.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-formParameterAnnotations")) {
        parsedOptions.formParameterAnnotations.addAll(asList(copyOfRange(
            option, 1, option.length)));
      } else if (option[0].equals("-formParameterTypes")) {
        parsedOptions.formParameterTypes.addAll(asList(copyOfRange(option, 1,
            option.length)));

      } else if (option[0].equals("-discriminatorAnnotations")) {
        parsedOptions.discriminatorAnnotations.addAll(asList(copyOfRange(
            option, 1, option.length)));
      } else if (option[0].equals("-subTypesAnnotations")) {
        parsedOptions.subTypesAnnotations.addAll(asList(copyOfRange(option, 1,
            option.length)));

      } else if (option[0].equals("-compositeParamAnnotations")) {
        parsedOptions.compositeParamAnnotations.addAll(asList(copyOfRange(
            option, 1, option.length)));
      } else if (option[0].equals("-compositeParamTypes")) {
        parsedOptions.compositeParamTypes.addAll(asList(copyOfRange(option, 1,
            option.length)));

      } else if (option[0].equals("-parameterNameAnnotations")) {
        parsedOptions.parameterNameAnnotations.addAll(asList(copyOfRange(
            option, 1, option.length)));
      } else if (option[0].equals("-stringTypePrefixes")) {
        parsedOptions.stringTypePrefixes.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-excludeClassTags")) {
        parsedOptions.excludeClassTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-excludeOperationTags")) {
        parsedOptions.excludeOperationTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-excludeFieldTags")) {
        parsedOptions.excludeFieldTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-excludeParamsTags")) {
        parsedOptions.excludeParamsTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-csvParamsTags")) {
        parsedOptions.csvParamsTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-paramsMinValueTags")) {
        parsedOptions.paramsMinValueTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-paramsMaxValueTags")) {
        parsedOptions.paramsMaxValueTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-paramsDefaultValueTags")) {
        parsedOptions.paramsDefaultValueTags.addAll(asList(copyOfRange(option,
            1, option.length)));
      } else if (option[0].equals("-paramsNameTags")) {
        parsedOptions.paramsNameTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-resourceTags")) {
        parsedOptions.resourceTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-responseTypeTags")) {
        parsedOptions.responseTypeTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-inputTypeTags")) {
        parsedOptions.inputTypeTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-defaultErrorTypeTags")) {
        parsedOptions.defaultErrorTypeTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-apiDescriptionTags")) {
        parsedOptions.apiDescriptionTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-operationNotesTags")) {
        parsedOptions.operationNotesTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-operationSummaryTags")) {
        parsedOptions.operationSummaryTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-fieldDescriptionTags")) {
        parsedOptions.fieldDescriptionTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-fieldMinTags")) {
        parsedOptions.fieldMinTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-fieldMaxTags")) {
        parsedOptions.fieldMaxTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-fieldDefaultTags")) {
        parsedOptions.fieldDefaultTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-requiredParamsTags")) {
        parsedOptions.requiredParamsTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-optionalParamsTags")) {
        parsedOptions.optionalParamsTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-requiredFieldTags")) {
        parsedOptions.requiredFieldTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-optionalFieldTags")) {
        parsedOptions.optionalFieldTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-unauthOperationTags")) {
        parsedOptions.unauthOperationTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-authOperationTags")) {
        parsedOptions.authOperationTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-unauthOperationTagValues")) {
        parsedOptions.unauthOperationTagValues.addAll(asList(copyOfRange(
            option, 1, option.length)));
      } else if (option[0].equals("-authOperationScopes")) {
        parsedOptions.authOperationScopes.addAll(asList(copyOfRange(option, 1,
            option.length)));
      } else if (option[0].equals("-operationScopeTags")) {
        parsedOptions.operationScopeTags.addAll(asList(copyOfRange(option, 1,
            option.length)));
    ...
    parsedOptions.recorder =
        new ObjectMapperRecorder(serializationFeaturesCsv,
            deserializationFeaturesCsv, defaultTyping, serializationInclusion);
  ...
  private List<String> unauthOperationTags; // tags that say a method does NOT
                                            // require authorization
  private List<String> authOperationTags; // tags that indicate whether an
                                          // operation requires auth or not,
                                          // coupled with a value from
                                          // unauthOperationTagValues
  private List<String> unauthOperationTagValues; // for tags in
                                                 // authOperationTags this is
                                                 // the value to look for to
                                                 // indicate method does NOT
                                                 // require
  // authorization
  private List<String> authOperationScopes; // default scopes to add if
                                            // authOperationTags is present but
                                            // no scopes
  private List<String> operationScopeTags; // explicit scopes that are required
                                           // for authorization for a method

  private List<String> resourcePriorityTags;
  private List<String> resourceDescriptionTags;

  private List<String> fileParameterAnnotations; // FQN of annotations that if
                                                 // present denote a parameter
                                                 // as being a File data type
  private List<String> fileParameterTypes; // FQN of types of a parameter that
                                           // are File data types

  private List<String> formParameterAnnotations; // FQN of annotations that if
                                                 // present denote a parameter
                                                 // as being a form parameter
                                                 // type
  private List<String> formParameterTypes; // FQN of types of a parameter that
                                           // are form parameter types

  private List<String> parameterNameAnnotations;

  private List<String> stringTypePrefixes; // list of type prefixes that are
                                           // mapped to string data type, can be
                                           // used for example to map header
                                           // types to
  // strings
  ...
  private NamingConvention modelFieldsNamingConvention =
      NamingConvention.DEFAULT_NAME;
  ...
  public DocletOptions() {
    ...
    this.excludeModelPrefixes
        .add("org.jboss.resteasy.plugins.providers.multipart.");
    ...
    this.fileParameterAnnotations
        .add("org.jboss.resteasy.annotations.providers.multipart.MultipartForm");
    ...
    this.fileParameterTypes
        .add("org.jboss.resteasy.plugins.providers.multipart.MultipartFormDataInput");
    ...
    this.formParameterTypes
        .add("com.sun.jersey.core.header.FormDataContentDisposition");
    ...
    this.discriminatorAnnotations
        .add("com.fasterxml.jackson.annotation.JsonTypeInfo");

    this.subTypesAnnotations = new ArrayList<String>();
    this.subTypesAnnotations
        .add("com.fasterxml.jackson.annotation.JsonSubTypes");
    ...
    for (String paramAnnotation : ParserHelper.JAXRS_PARAM_ANNOTATIONS) {
      fnnTranslator.addNext(new AnnotationAwareTranslator(this).element(
    ...
    fnnTranslator
        ...
        .addNext(
            new AnnotationAwareTranslator(this).ignore(
                "javax.xml.bind.annotation.XmlTransient").element(
        ...
        .addNext(
            new AnnotationAwareTranslator(this)
                ...
                .element("com.fasterxml.jackson.annotation.JsonProperty",
                    "value")
                .rootElement("com.fasterxml.jackson.annotation.JsonRootName",
                    "value"))
        .addNext(
            new AnnotationAwareTranslator(this)
                .ignore("org.codehaus.jackson.map.annotate.JsonIgnt core")
                .element("org.codehaus.jackson.map.annotate.JsonProperty",
                    "value")
                .rootElement("org.codehaus.jackson.map.annotate.JsonRootName",
                    "value")).addNext(new NameBasedTranslator(this));
   ...
   * 
   * @param excludeResourcePrefixes the prefixes of the FQN of resource classes
   *        to exclude
   * @return this
   */
  public DocletOptions setExcludeResourcePrefixes(
      List<String> excludeResourcePrefixes) {
  ...
  /**
   * This gets a list of FQN of types which simply wrap the real underlying data
   * type
   * 
   * @return The type
   */
  ...
  /**
   * This is whether model fields are required by default e.g. if it is not
   * specified whether a field is optional or not
   * 
   * @return whether model fields are required by default e.g. if it is not
   *         specified whether a field is optional or not
   */
   ...
   * @return this
   */
  public DocletOptions setModelFieldsRequiredByDefault(
      boolean modelFieldsRequiredByDefault) {
   ...
   * 
   * @param modelFieldsXmlAccessTypeEnabled the modelFieldsXmlAccessTypeEnabled
   *        to set
   * @return this
   */
  public DocletOptions setModelFieldsXmlAccessTypeEnabled(
      boolean modelFieldsXmlAccessTypeEnabled) {
   ...
   * @return this
   */
  public DocletOptions setModelFieldsNamingConvention(
      NamingConvention modelFieldsNamingConvention) {
  ...
  /**
   * This gets a list of javadoc tag names that can be used for the api
   * description
   * 
  ...
  /**
   * This gets a list of javadoc tag names that can be used for the operation
   * notes
   * 
   ...
   * summary
   * 
   * @return a list of javadoc tag names that can be used for the operation
   *         summary
   */
  ...
  /**
   * This gets list of javadoc tag names that can be used for the model
   * field/method descriptions
   * 
   * @return list of javadoc tag names that can be used for the model
   *         field/method descriptions
   */
  ...
  /**
   * This gets list of javadoc tag names that can be used for ordering resources
   * in the resource listing
   * 
   * @return the list of javadoc tag names that can be used for ordering
   *         resources in the resource listing
   */
  ...
  /**
   * This gets list of javadoc tag names that can be used for the description of
   * resources
   * 
   * @return the list of javadoc tag names that can be used for the description
   *         of resources
   */
   ...
   * @return this
   */
  public DocletOptions setSortResourcesByPriority(
      boolean sortResourcesByPriority) {
   ...
   * @return this
   */
  public DocletOptions setExcludeDeprecatedOperations(
      boolean excludeDeprecatedOperations) {
   ...
   * @return this
   */
  public DocletOptions setExcludeDeprecatedFields(
      boolean excludeDeprecatedFields) {
   ...
   * @return this
   */
  public DocletOptions setExcludeDeprecatedParams(
      boolean excludeDeprecatedParams) {
   ...
   * @return this
   */
  public DocletOptions setResponseMessageSortMode(
      ResponseMessageSortMode responseMessageSortMode) {
  ...
  /**
   * This replaces any variables in the given value with replacements defined in
   * the doclets variable replacements file
   * 
  ...
  public String replaceVars(String value) {
    if (value != null && this.variableReplacements != null
        && !this.variableReplacements.isEmpty()) {
      return VariableReplacer
          .replaceVariables(this.variableReplacements, value);
		}
		return value;
	}
